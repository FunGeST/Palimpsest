...
resdir_parent <- paste0("~/Documents/Results/delet/Palimpsest2_test_script_",gsub("-","",Sys.Date()),"b/");if(!file.exists(resdir_parent))	dir.create(resdir_parent) ## Path to parent results directory
...
vcf <- load2object("/Volumes/HEPATO PARTAGE/GENOMIC_DATA/WGS/RData/Somatic_mutations/140s_113p_Oncotated_Lifted_Somatic_variants_filtered_main_columns_fully_annotated.RData") %>% 
  ...
  filter(Sample %in% unique(Sample)[1:5]) %>% 
...
SBS_denovo_sigs <- NMF_Extraction(input_matrices = SBS_input, range_of_sigs = 1:10,nrun = 10,resdir = resdir, num_of_sigs = 5)
