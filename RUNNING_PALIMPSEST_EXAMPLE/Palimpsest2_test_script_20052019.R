...
vcf <- load2object("/Volumes/HEPATO PARTAGE/GENOMIC_DATA/WGS/RData/Somatic_mutations/140s_113p_Oncotated_Lifted_Somatic_variants_filtered_main_columns_fully_annotated.RData") %>% 
  ...
  filter(Sample %in% unique(Sample)[1:10]) %>% 
...
SBS_denovo_sigs <- NMF_Extraction(input_matrices = SBS_input, range_of_sigs = 3:10,nrun = 5,resdir = resdir)
...
SBS_signatures_exp <- deconvolution_fit2(input_matrices = SBS_input,input_signatures = SBS_denovo_sigs,threshold = 6,resdir = resdir, sig_cols = SBS_col)
...
# calculate and plot the exposure of the signatures across the series
SBS_signatures_exp <- deconvolution_fit2(input_matrices = SBS_input, input_signatures = SBS_liver_sigs, threshold = 6, sig_cols = sig_cols,resdir = resdir)
...
# calculate and plot the exposure of the signatures across the series
DBS_signatures_exp <- deconvolution_fit2(input_matrices = DBS_input, input_signatures = DBS_liver_sigs, threshold = 6, sig_cols = sig_cols,resdir = resdir)
...
# calculate and plot the exposure of the signatures across the series
ID_signatures_exp <- deconvolution_fit2(input_matrices = ID_input, input_signatures = ID_liver_sigs, threshold = 6, sig_cols = sig_cols,resdir = resdir)
...
SBS_input_clonal <- Palimpsest_input2(vcf = vcf.clonal,Type = "SBS")
signatures_exp_clonal <- deconvolution_fit2(input_matrices = SBS_input_clonal,input_signatures = SBS_liver_sigs,threshold = 6, resdir =  resdir,save_signatures_exp = F)

vcf.subclonal <- vcf_cna[which(vcf_cna$Clonality=="subclonal"),]
SBS_input_subclonal <- Palimpsest_input2(vcf = vcf.subclonal,Type = "SBS")
signatures_exp_subclonal <- deconvolution_fit2(input_matrices = SBS_input_subclonal,input_signatures = SBS_liver_sigs,threshold = 6, resdir =  resdir,save_signatures_exp = F)
...
SV.vcf <- preprocessInput_sv_2(input_data =  SV_data,resdir = resdir)
SV_input <- Palimpsest_input2(vcf = SV.vcf,Type = "SV")
...
SVsignatures_exp <- deconvolution_fit_SV(vcf = SV.vcf,type = "SV",input_data = SV_input$mut_props,threshold = 6,input_signatures = SV_denovo_sigs,
...
signature_content_plot <- deconvolution_exposure(SVsignatures_exp$sig_nums,SVsignatures_exp$sig_props,sig_cols = SV_cols)
