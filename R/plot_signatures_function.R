  ...
  for (i in 1:num_of_sigs) {
    ...
    if (Type == "DBS" & label == "Full"){
      ...
      plot_output = plot_output +
        ...
        annotate("text",x = 39, y =   0-max.y*0.14, label = "Context", size = 7, fontface = "plain", colour = "black") +
    ...
    if (Type == "SBS"){
      if(label == "Full"){
        ...
        plot_output = plot_output +
          ...
          annotate("text",x = 48, y =   0-max.y*0.22, label = "Context", size = 8, fontface = "plain", colour = "black") +
          theme(axis.text.x = element_text(angle = 90, vjust=0.5,  hjust = 0, size = 20, colour = "grey47", family = "mono"), 
                axis.ticks.x = element_line(),
                axis.text.y = element_text(size = 20),
                axis.ticks.y.right = element_blank()
    ...
    if (Type == "SV" & label == "Full"){
      stop("I haven't finished this bit yet lol")
      plot_output = plot_output +

        xlab(" ") + ylab(" ") +
        scale_x_discrete("", labels = plot_input$Length) +
        geom_segment(aes(x = 19.5, y = 0, xend = 19.5, yend = max.y*1.1)) +
        annotate("text",x = -1, y = max.y*0.66, label = "Proportion of Mutations", angle = 90, size = 7, fontface = "plain", colour = "black") +
        annotate("text",x = 19, y =   0-max.y*0.35, label = "Size", size = 10, fontface = "plain", colour = "black") +
        theme(axis.text.x = element_text(angle = 90, vjust=0.5,   hjust = 0.9, 
                                         size = 15, colour = "grey47"), 
              plot.margin=unit(c(1,1,1,1.5),"cm"), # order = top, right, bottom, left
              axis.ticks.x = element_line(),
              plot.title = element_text(vjust = -30)) +
        
        geom_rect(mapping = aes(xmin = (plot_input$Start_pos - 0.5), ymin = max.y*1.25, xmax = (plot_input$Start_pos  + xmax_correction),
                                ymax= max.y*1.375), fill = plot_input$Colours, color = "white", size = 2.5) +
        geom_text(position = "identity", mapping = aes(x = plot_input$Start_pos + (plot_input$Substype_length/2.5), 
                                                       y = max.y*1.285, label = plot_input$Substype), size = 6) +
        scale_y_continuous(expand = c(0, 0), limits = c(-1, max.y*1.3)) #+
    } 
    if(label=="None"){
      plot_output = plot_output +
